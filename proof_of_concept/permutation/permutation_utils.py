...
def _get_permutation_group(
    generator_cycles_list: Iterable[Iterable[Iterable[int]]], n_qubits: int
) -> Set[Tuple[int, ...]]:
    """
    Generates the full permutation group from a set of generator permutations.
    Each generator is in cycle format (e.g., [[0, 1], [2, 3]]).
    """
    identity_map = tuple(range(n_qubits))
    generator_maps = [_cycles_to_map(cycles, n_qubits) for cycles in generator_cycles_list]
    
    group_elements: Set[Tuple[int, ...]] = {identity_map}
    frontier: deque[Tuple[int, ...]] = deque(generator_maps)
    group_elements.update(generator_maps)

    while frontier:
        current_map = frontier.popleft()
        
        for gen_map in generator_maps:
            new_map = _compose_maps(gen_map, current_map)
...
def are_generators_equivalent(
    ...
    - Single generator as list of cycles: [[1, 2], [3, 4]]
    - Nested generator format: [[[1, 2], [3, 4]]]
    """
    # Standardize input: flatten if we have nested list of generators
    def standardise_cycles(gen):
        gen_list = list(gen)
        if not gen_list:
            return []
        
        # Check if this is a nested structure (list of lists of lists)
        # by checking if the first element is itself a list of lists
        if gen_list and isinstance(gen_list[0], (list, tuple)):
            first_elem = gen_list[0]
            if first_elem and isinstance(first_elem[0], (list, tuple)):
                # This is nested [[[]]], flatten one level
                return gen_list[0]
        return gen_list
    
    gen1_std = standardise_cycles(gen1)
    gen2_std = standardise_cycles(gen2)
    
    map1 = _cycles_to_map(gen1_std, n_qubits)
    map2 = _cycles_to_map(gen2_std, n_qubits)
    
    return map1 == map2

def find_minimal_generators(
    all_symmetries_cycles: Iterable[Iterable[Iterable[int]]], n_qubits: int
) -> List[Iterable[Iterable[int]]]:
    """
    Sieves a list of all group elements to find a minimal generating set.

    Args:
        all_symmetries_cycles: A list of *all* non-identity symmetries
                                (e.g., your list of 7), in cycle format.
        n_qubits: The number of qubits.

    Returns:
        A minimal set of generators, also in cycle format.
    """
    identity_map = tuple(range(n_qubits))
    
    # The set of generators we are building
    minimal_generators_cycles: List[Iterable[Iterable[int]]] = []
    
    # The group generated by the generators found so far
    generated_group_maps: Set[Tuple[int, ...]] = {identity_map}

    # Iterate through each permutation you found
    for candidate_cycles in all_symmetries_cycles:
        candidate_map = _cycles_to_map(candidate_cycles, n_qubits)

        # Check if this permutation is *already* generated by our set S
        if candidate_map not in generated_group_maps:
            # It's not. This element is "new".
            # We MUST add it to our set of generators.
            minimal_generators_cycles.append(candidate_cycles)

            # Now, we must update the generated_group_maps to be
            # the full group generated by our *new* set of generators.
            generated_group_maps = _get_permutation_group(
                minimal_generators_cycles, n_qubits
            )
            
    return minimal_generators_cycles
